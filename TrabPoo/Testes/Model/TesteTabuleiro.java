...
// 		inicializa o tabuleiro, cria os jogadores e move inicializa 2 peças do jogador 1
 		...
 		t.setPecaCasaDeSaida(j1.getPeca(-1));
 		t.movePeca(j1, 39, 3);// é preciso mover uma peça da casa de saída antes de colocar outra
 		t.movePeca(j1, -1, 5);
 		t.setPecaCasaDeSaida(j2.getPeca(-1));
...
// 		colocando a peça do j2 atrás e tenta fazer a peça do jogador 2 ultrapassar a barreira
 		t.movePeca(j2, 26, 14);
 		t.movePeca(j2,40, 4);
 		assertTrue("O jogador ultrapassou a barreira",j2.getPeca(41) != null );
...
// 	Testar se o jogador consegue ir para um índice maior que 52 (fora do tabuleiro)
 	...
 	public void testa_peca_fora_tabuleiro() {
		Tabuleiro t = new Tabuleiro();
 		...
 		t.setPecaCasaDeSaida(j.getPeca(-1));
 		t.movePeca(j, 39 , 13);
 		assertTrue("A peça do jogador saiu do tabuleiro",j.getPeca(0) != null);
...
// 	Testando se o peão é capturado 
 	...
 	public void teste_peao_come() {
 		...
 		t.movePeca(j2, 13, 1);
 		t.movePeca(j1, 0, 13);
 		assertTrue("A peça não é capturado quando cai na mesma casa",j2.verificaSeTemPeca());
 	}
// 	Testando se a peça chega na reta final
 	@Test
 	public void teste_reta_final() {
 		Tabuleiro t = new Tabuleiro();
 		Jogador j1 = new Jogador("Teste",Cores.AZUL);
 		...
 		assertTrue("A peça se moveu com um dado diferente da distância para a casa finall",j1.getPeca(2).isRetaFinal());
